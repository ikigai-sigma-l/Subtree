...
function update(name) {
  ...
  if (json.dependencies) {
    ...
    for (let i = 0; i < keys.length; i++) {
      if (/^@ikigaians\/.*/.test(keys[i])) {
        const dependency = keys[i].replace('@ikigaians/ikgplayer-', '')
        json.dependencies[keys[i]] = "file:" + output + "/ikigaians-ikgplayer-" +  dependency + "-" + packages[dependency].version + ".tgz"
      }
    }
  }
